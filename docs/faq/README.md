...
IOS 的 UIWebview 内核 设定了其在进行 momentum scrolling(弹性滚动)时,会停止所有的 事件响应 及 DOM 操作引起的页面渲染 (亲测),故 onscroll 不能实时响应。
...
但注意，该属性在 IOS 下兼容性不错，但是 android 和 PC 下 兼容性堪忧，注意分平台做兼容。
...
这是是由于 IOS 的 safari 对于 web 页面，有一个双击缩放页面的功能。
...
在 IOS 中，input 的 placeholder 提示文案，当 input 设置 行高后， 提示文案会稍微偏上（chrome 浏览器和 android 没有问题）
